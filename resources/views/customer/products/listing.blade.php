        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-1/4">
                <div class="bg-white rounded-xl shadow p-5 sticky top-6 filter-section">
                    ...
                    <!-- Attribute Filters (e.g., Metal Type, Stone Type) -->
                    ...
                    <!-- Specification Filters (e.g., Material, Occasion, Style) -->
                ...
                @if (count($products ?? []) > 0)
                    <div id="productsContainer" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($products as $product)
                            <div
                                class="product-card bg-white rounded-xl shadow overflow-hidden group hover:shadow-lg transition-all duration-300 h-full flex flex-col">
                                <!-- Product Image -->
                                <div class="relative aspect-[4/5] overflow-hidden">
                                    ...
                                    <!-- Quick Actions -->
                                    <div
                                        class="absolute bottom-3 left-3 right-3 flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                        <button type="button"
                                            class="add-to-wishlist bg-white p-2 rounded-full shadow hover:bg-amber-50 hover:scale-110 transition-all"
                                            title="Add to Wishlist">
                                            <i class="fas fa-heart text-gray-700"></i>
                                        </button>
                                        <button type="button"
                                            class="quick-view bg-white p-2 rounded-full shadow hover:bg-amber-50 hover:scale-110 transition-all"
                                            data-product-slug="{{ $product['slug'] }}" title="Quick View">
                                            <i class="fas fa-eye text-gray-700"></i>
                                        </button>
                                        <button type="button"
                                            class="add-to-cart bg-white p-2 rounded-full shadow hover:bg-amber-50 hover:scale-110 transition-all"
                                            title="Add to Cart" {{ !$product['is_in_stock'] ? 'disabled' : '' }}>
...
@push('scripts')
    <script>
        ...
        function setViewMode(mode) {
            ...
            if (mode === 'grid') {
                ...
                productsContainer.querySelectorAll('.product-card').forEach(card => {
                    card.classList.add('flex-row', 'h-auto');
                    card.classList.remove('flex-col');

                    const image = card.querySelector('.relative.aspect-\\[4\\/5\\]');
                    const details = card.querySelector('.p-4');

                    if (image && details) {
                        image.classList.remove('aspect-[4/5]');
                        image.classList.add('w-1/3', 'h-64');
                        details.classList.add('w-2/3');
                    }
                });
            }
        ...
        async function quickView(productSlug) {
            try {
                const response = await fetch(`/products/${productSlug}/quick-view`);
