...
@section('meta_description',
    ...
    ?? 'Discover our exquisite collection of jewellery crafted with precision and passion.'
)
...
@section('og_description',
    $category->meta_description
    ?? 'Premium quality jewellery collection with genuine precious metals and gemstones.'
)

@section('og_image',
    $category->featured_image
        ? asset($category->featured_image)
    ...
    <div class="max-w-7xl mx-auto px-4 relative z-10">
            ...
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    <li class="inline-flex items-center">
                        <a href="{{ route('customer.home.index') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-amber-600 transition-colors">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('customer.products.list') }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-amber-600 md:ml-2 transition-colors">
                                Categories
                            </a>
                        </div>
                    </li>
        ...
        <div class="text-center">
            ...
            <div class="flex flex-col items-center justify-center mb-8">
                <div class="w-24 h-24 bg-gradient-to-br from-amber-100 to-amber-200 rounded-full flex items-center justify-center shadow-lg mb-6 animate-bounce-in">
                ...
                <h1 class="brand-title text-5xl md:text-6xl text-gray-800 mb-4 animate-fade-in-up" style="animation-delay: 0.2s;">
            ...
            <!-- Category Stats -->
            <div class="flex flex-wrap justify-center gap-8 mb-12 animate-fade-in-up" style="animation-delay: 0.6s;">
                <div class="text-center">
...
<section class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-12">
            <div>
                <h2 class="brand-title text-3xl md:text-4xl text-gray-800 mb-2">All {{ $category->name ?? 'Products' }}</h2>
                <p class="text-gray-600">Showing {{ $paginator['from'] ?? 0 }}-{{ $paginator['to'] ?? 0 }} of {{ $paginator['total'] ?? 0 }} products</p>
            </div>

            <!-- Sort Options -->
            <div class="mt-4 md:mt-0">
                <select id="sortBy" onchange="updateSorting()" class="px-4 py-2 rounded-full border border-amber-200 bg-white focus:outline-none focus:ring-2 focus:ring-amber-200 transition-all hover:shadow-lg">
        ...
        <div class="mb-8 p-4 bg-amber-50 rounded-2xl">
            <div class="flex flex-wrap gap-4">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="number" id="maxPrice" placeholder="Max" value="{{ $maxPrice ?? '' }}"
                               class="w-24 px-3 py-1 rounded-lg border border-amber-200 focus:outline-none focus:ring-2 focus:ring-amber-200">
                        <button onclick="applyPriceFilter()" class="px-4 py-1 bg-amber-600 text-white rounded-lg hover:bg-amber-700">
                            Apply
                        </button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select id="inStock" onchange="applyInStockFilter()" class="px-3 py-1 rounded-lg border border-amber-200">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            @forelse($products as $product)
                <div class="product-card bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 group h-full flex flex-col animate-scale-in">
                    <!-- Product Image -->
                    <a href="{{ route('customer.products.details', $product['slug']) }}" class="block relative overflow-hidden">
...
/>
                            ...
                            @if($product['discount_percent'] > 0)
                                <div class="absolute top-4 right-4 bg-gradient-to-r from-amber-600 to-amber-800 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg">
                            ...
                            <!-- New Badge -->
                            @if($product['is_new'])
                                <div class="absolute top-4 right-4 bg-gradient-to-r from-green-600 to-green-800 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg">
                                    NEW
                                </div>
                            @endif
                            ...
                            <!-- Wishlist Button -->
                            <button type="button"
                                    class="absolute top-4 left-4 w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-lg hover:bg-amber-50 transition-all opacity-0 group-hover:opacity-100 hover:scale-110 wishlist-btn"
                    ...
                    <div class="p-6 flex-grow flex flex-col">
                        ...
                        <a href="{{ route('customer.products.details', $product['slug']) }}" class="block group">
                            <h3 class="font-semibold text-gray-800 mb-3 text-lg group-hover:text-amber-700 transition-colors cursor-pointer line-clamp-2 hover:underline">
                                {{ $product['name'] }}
                            </h3>
                        </a>
                        ...
                        <div class="mt-auto">
                            <div class="flex items-center gap-2 mb-4">
                                ...
                                @if($product['compare_price'])
                                    <p class="text-sm text-gray-400 line-through hover:text-gray-600 transition-colors">
                            ...
                            <div class="flex gap-2">
                                <button class="flex-1 bg-gray-900 text-white py-3 rounded-full font-semibold hover:bg-gray-800 transition-all duration-300 transform hover:scale-105 text-sm add-to-cart-btn hover:shadow-lg"
...
<section class="py-16 bg-gradient-to-b from-white to-amber-50">
    <div class="max-w-7xl mx-auto px-4">
        <div class="text-center mb-12">
            <h2 class="brand-title text-3xl md:text-4xl text-gray-800 mb-4">Why Choose Our {{ $category->name ?? 'Jewellery' }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <!-- Feature 1 -->
            <div class="text-center p-8 rounded-3xl bg-white shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 animate-slide-up" style="animation-delay: 0.1s;">
                <div class="w-20 h-20 mx-auto mb-6 bg-gradient-to-br from-amber-100 to-amber-200 rounded-2xl flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-500">
                    <i class="fas fa-award text-3xl text-amber-700"></i>
                </div>
                <h3 class="text-xl font-bold text-gray-800 mb-4 hover:text-amber-700 transition-colors">Authentic Materials</h3>
            ...
            <!-- Feature 2 -->
            <div class="text-center p-8 rounded-3xl bg-white shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 animate-slide-up" style="animation-delay: 0.2s;">
                <div class="w-20 h-20 mx-auto mb-6 bg-gradient-to-br from-amber-100 to-amber-200 rounded-2xl flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-500">
                    <i class="fas fa-hands text-3xl text-amber-700"></i>
                </div>
                <h3 class="text-xl font-bold text-gray-800 mb-4 hover:text-amber-700 transition-colors">Handcrafted</h3>
                <p class="text-gray-600">
            ...
            <!-- Feature 3 -->
            <div class="text-center p-8 rounded-3xl bg-white shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 animate-slide-up" style="animation-delay: 0.3s;">
                <div class="w-20 h-20 mx-auto mb-6 bg-gradient-to-br from-amber-100 to-amber-200 rounded-2xl flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-500">
                    <i class="fas fa-shield-alt text-3xl text-amber-700"></i>
                </div>
                <h3 class="text-xl font-bold text-gray-800 mb-4 hover:text-amber-700 transition-colors">Lifetime Warranty</h3>
                <p class="text-gray-600">
                    Confidence in every purchase with our comprehensive lifetime warranty.
                </p>
            </div>
        </div>
    </div>
...
function showNotification(message) {
    ...
    notification.className = 'fixed top-4 right-4 bg-green-600 text-white px-6 py-3 rounded-full shadow-lg z-50';
