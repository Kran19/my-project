...
<div class="mb-8">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 mb-2">Edit Home Section</h2>
...
<div class="max-w-4xl">
    <form action="{{ route('admin.home-sections.update', $section->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" onchange="toggleTypeFields()" 
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition">
                        <option value="category" {{ old('type', $section->type) == 'category' ? 'selected' : '' }}>Category Based</option>
                        <option value="custom_products" {{ old('type', $section->type) == 'custom_products' ? 'selected' : '' }}>Custom Product Selection</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="style" id="style" 
                        ...
                        @foreach($styles as $value => $label)
                            <option value="{{ $value }}" {{ old('style', $section->style) == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <!-- Category Field -->
                <div id="category-field-container" class="{{ old('type', $section->type) == 'custom_products' ? 'hidden' : '' }}">
                ...
                <div id="products-field-container" class="md:col-span-2 {{ old('type', $section->type) == 'custom_products' ? '' : 'hidden' }}">
                ...
                <div class="flex items-center pt-6">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="status" value="1" class="sr-only peer" {{ $section->status ? 'checked' : '' }}>
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition font-bold shadow-md shadow-indigo-100">Update Section</button>
        </div>
    </form>
</div>
@endsection
